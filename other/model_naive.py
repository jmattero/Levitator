...
class QNetwork(nn.Module):
    ...
    def forward(self, state, action):
        ...
        x2 = F.relu(self.ln1(self.linear4(xu)))
        x2 = F.relu(self.ln2(self.linear5(x2)))
        x2 = self.linear6(x2)
...
class NaivePolicy():
    def __init__(self, initial, target, max_steps):
        ...
        #print(self.max_updates)

    def sample(self, state):
        self.l = 0.0005*(torch.rand(1)+0.5)
        #print(self.previous, self.l*self.fixed_direction_vec)
        if self.updates < self.max_updates:
        #    self.action_point = self.previous + self.l * self.fixed_direction_vec
