...
warmup_naive = False
#naive_delay = 10
naive = [initial, target, max_steps/naive_delay]
if warmup_naive:
    #max_steps =  np.ceil(np.linalg.norm(target - initial[:3]) / l * warmup_delay)
    print('--- max Naive steps: ', max_steps)
    #args.start_steps = max_steps
    args.env_name = 'Levitator_warmup'
...
# Agent
agent = SAC(env.observation_space.shape[0], env.action_space, args, naive, warmup_naive)
...
for i_episode in itertools.count(1):
    ...
    while not done:
        if episode_steps % delay == 0 or episode_steps == 0:
            if args.policy == 'Naive':
                action = agent.select_naive(state) # select naive action
            elif args.start_steps > total_numsteps:  #first 10 000 (if default) steps:
                if warmup_naive:
                    action = agent.select_naive(state) # select naive action 
                else:
                    # if episode_steps == 0:
                   ...
                   # else:
                    action = state[:3] + env.action_space.sample()/10  # Sample random action
    ...
    # #add naive trajectories to the replay buffer
    # if i_episode % naive_to_memory == 0:
    #     #print("add niave to replay buffer")
    #     #select action from hardcoded policy:
    #     state = env.reset()[0]
    #     done = False
    ...
    #         memory.push(state, action, reward, next_state, mask)
        # if episode_steps > 100000:
        #     naive_to_memory = 5000
